...
const C = {
  ...
  purple: '#f5f3ff', purpleText: '#5b21b6', purpleBorder: '#c4b5fd',
...
export default function App() {
  ...
  const [scheduleData] = useState(RAW_SCHEDULE);
  ...
  const coaches = useMemo(() => buildCoaches(scheduleData), [scheduleData]);
  const ff = (k, v) => setForm(p => ({ ...p, [k]: v }));

  // ── Date helpers ──
  const baseDate = useMemo(() => {
    const d = new Date('2026-04-26T12:00:00'); d.setDate(d.getDate() + weekOffset * 7); return d;
  }, [weekOffset]);

  const weekDates = useMemo(() => {
    const m = {}; DAY_ORDER.forEach((d, i) => { const dt = new Date(baseDate); dt.setDate(baseDate.getDate() + i); m[d] = dt.toISOString().slice(0, 10); }); return m;
  }, [baseDate]);

  const fmtRange = () => { const end = new Date(baseDate); end.setDate(baseDate.getDate() + 6); return `${baseDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} – ${end.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}`; };

  // ── Schedule helpers ──
  const instructorSchedule = useMemo(() => form.instructorName ? scheduleData.filter(r => r.name === form.instructorName) : [], [form.instructorName, scheduleData]);
  const availableDays = useMemo(() => [...new Set(instructorSchedule.map(r => r.day).filter(d => DAY_ORDER.includes(d)))].sort((a, b) => DAY_ORDER.indexOf(a) - DAY_ORDER.indexOf(b)), [instructorSchedule]);
  ...
  // Auto-refresh every 30 seconds so shifts update in real time
  useEffect(() => {
  ...
  const postShift = async () => {
    ...
    try {
      ...
      // Find eligible coaches (available that day, not teaching at same time)
      ...
      if (eligible.length > 0) {
        ...
        setSmsLog(p => [...notifyData.results.map(r => ({ to: r.coach, success: r.success, time: new Date().toLocaleTimeString() })), ...p]);
      }
  ...
  const claimShift = async (shiftId) => {
    ...
    try {
      const res = await fetch('/api/claim', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ shift_id: shiftId, action: 'claim', coach_id: activeCoach.id, coach_name: activeCoach.name }),
      });
  ...
  const confirmShift = async (shiftId) => {
    ...
    try {
      const res = await fetch('/api/claim', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ shift_id: shiftId, action: 'confirm' }),
      });
  ...
  const assignSub = async (shift, coach) => {
    ...
    try {
      const res = await fetch('/api/claim', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ shift_id: shift.id, action: 'claim', coach_id: coach.id, coach_name: coach.name }),
      });
      if (!res.ok) throw new Error('Failed to assign sub');
      await fetchShifts();
  ...
  const toggleDay = (day) => { if (!activeCoach) return; setAvailability(p => { const c = p[activeCoach.id] || []; return { ...p, [activeCoach.id]: c.includes(day) ? c.filter(d => d !== day) : [...c, day] }; }); };
  ...
  return (
    <div style={{ padding: '1rem', fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif", background: C.bg2, minHeight: '100vh', boxSizing: 'border-box' }}>
      ...
      <div style={{ background: C.bg, borderRadius: C.radius, border: `1px solid ${C.border}`, padding: '12px 16px', marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
        <div>
          <div style={{ fontWeight: 700, fontSize: 17, color: C.text }}>🤸 Bay Aerials</div>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 16 }}>
            {[['Open', shifts.filter(s => s.status === 'open').length, C.yellow, C.yellowText, C.yellowBorder],
              ...
              <div key={l} style={{ background: bg, border: `1px solid ${bc}`, borderRadius: C.radius, padding: '12px', textAlign: 'center' }}>
          ...
          {adminTab === 'calendar' && (
            <div style={{ background: C.bg, borderRadius: C.radius, border: `1px solid ${C.border}`, padding: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 5, marginBottom: 12, flexWrap: 'wrap' }}>
                {DAY_ORDER.map(day => (
                  <button key={day} onClick={() => setPrintDay(printDay === day ? null : day)} style={{ background: printDay === day ? C.blue : C.bg2, border: `1px solid ${printDay === day ? C.blueBorder : C.border}`, color: printDay === day ? C.blueText : C.text2, borderRadius: C.radiusSm, padding: '3px 9px', fontSize: 11, cursor: 'pointer', fontWeight: printDay === day ? 600 : 400 }}>🖨 {DAY_SHORT[day]}</button>
                ))}
              </div>
              ...
              {printDay && (() => {
                ...
                const lines = [
                  `BAY AERIALS — ${printDay.toUpperCase()} SCHEDULE`,
                  dateStr, '─'.repeat(56),
                  ...rows.map(r => {
                    const s = r.subShift;
                    const subName = s?.claimedByName ? shortName(s.claimedByName) : null;
                  ...
                  '─'.repeat(56), `Bay Aerials Gymnastics · ${new Date().toLocaleString()}`,
                ].join('\n');
                return (
                  <div style={{ marginTop: 16, border: `1px solid ${C.blueBorder}`, borderRadius: C.radius, overflow: 'hidden' }}>
                    <div style={{ background: C.blue, padding: '10px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
          ...
          {adminTab === 'sms' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {smsLog.length === 0 && <div style={{ ...card, textAlign: 'center', color: C.text2, padding: '2rem' }}>No messages yet. Post a shift to trigger SMS notifications.</div>}
              {smsLog.map((m, i) => (
      ...
      {view === 'coach' && !activeCoach && (
        <div style={{ maxWidth: 320, margin: '0 auto', paddingTop: '1rem' }}>
          <div style={{ ...card, textAlign: 'center' }}>
            ...
            <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 6 }}>Coach Portal</div>
      ...
      {view === 'coach' && activeCoach && (
        <div>
          ...
          {coachTab === 'avail' && (
            <div style={card}>
              <div style={{ fontSize: 13, color: C.text2, marginBottom: 12 }}>Tap days you're available. Admin uses this to notify the right coaches.</div>
          ...
          <div style={{ background: C.bg, borderRadius: C.radius, border: `1px solid ${C.border}`, width: 480, maxWidth: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ padding: '20px 24px' }}>
              {[
                ...
                { n: '06', title: 'If uncovered', desc: "If no one can cover: Manager covers personally OR notify families 60+ min before class with credit/makeup option." },
                { n: '07', title: 'Post-class', desc: 'Sub logs hours in iClassPro same as a normal shift. Manager notes the swap for the weekly payroll review.' },
              ].map(step => (
